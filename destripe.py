...
class destripe:
	...
	def __init__(self, dataset, Niter, a, wedgeSize, theta, kmin):
		if len(dataset.shape) == 3:
			self.dataset = np.mean(dataset,axis=2).astype(np.int64)
		else:
		...
		self.theta = theta
		self.kmin = kmin
		self.fftw = FFTW.WrapFFTW(self.dataset.shape)
		self.fft_raw = None
		self.ax_list = None
		

	def TV_reconstruction(self): 
		...
		self.recon_constraint = recon_constraint/np.amax(recon_constraint)*255
		

		self.recon_fft = np.log(np.abs(fftshift(self.fftw.fft(recon_constraint))) + 1)
	...
	def view_missing_wedge(self):
		...
		# fig, self.ax_list = plt.subplots(2,1, figsize=(14,6))
